...
# def call_GUI2():
#     #win2 = Toplevel(root)
#     print("FILENAME IN GUI2 IS=",filename)
#     GUI2.GUI21()
#     return
...
def call_resize():
    ...
    #print("shirt path", filename)
    # resizeinputargs.resizeinput(img,filename)
...
col_files = [[sg.Listbox(values=fnames, change_submits=True, size=(60, 30), key='listbox')],
             ...
             sg.Button('Continue to Select Person', size=(13, 3),button_color=('white', 'darkorange1')), file_num_display_elem]]
...
while True:
    ...
    if event == sg.WIN_CLOSED:
        ...
        if i < 0:
            i = num_files + i
        filename = os.path.join(folder, fnames[i])
    elif event in ('Continue to Select Person'):
